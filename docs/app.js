...
function marketCard(m){
  return `<article class="card">
    <div class="stock-head"><strong>${fmt(m.symbol)}</strong><span class="name">${fmt(m.name)}</span></div>
...
async function main(){
  try{
    ...
    $("appState").textContent=fmt(status.app_state_ja||status.run_state||status.app_state);
    $("tradeDate").textContent=fmt(status.us_trade_date);
    $("updatedAt").textContent=fmt(status.generated_at_jst);
    $("statusMessage").textContent=fmt(status.message);
    $("summary").textContent=market.summary||"データ接続前です。";
